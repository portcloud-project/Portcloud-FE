...
const fetchCommentViewLogs = async (id: string | string[]): Promise<CommentView[]> => {
    try {
        const response = await axios.get<ApiResponse<CommentView[]>>(
            `/api/comment-get-logs?id=${id}`,
        );
...
export const useCommentViewLogs = (id: string | string[]) => {
    return useQuery<CommentView[], Error>({
        queryKey: ['comment-view-logs', id],
        queryFn: () => fetchCommentViewLogs(id),
    });
};
